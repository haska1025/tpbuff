...
static int tp_gen_java_import_path(FILE *out, struct generated_file *gfile)
{
    ...
    for (inc = inc_tab; inc != NULL; inc=inc->next){
        if (inc->is_quote){
            char *pkgname = tpp_get_package_name_by_filename(inc->filename);
            pkgname = pkgname?pkgname:"tpp";
            fprintf(out, "import %s.*;\n", pkgname);
        }else if( 0 == strcmp(inc->filename, "vector")){
...
static int tp_gen_java_proto_decl(const char *save_dir, struct protocol *p, struct generated_file *gfile)
{
    char *tmp_dir = NULL;
    char *old_dir = NULL;
    FILE *javafile = NULL;
    ...
    char *pkgname = gfile->packagename;
    pkgname = pkgname?pkgname:"tpp";
    // Generate file header
    fprintf(javafile, "/// Generated by tpp compiler. Don't edit!\n");
    fprintf(javafile, "package %s;\n", pkgname);

    // Generate import file
    if (tp_gen_java_import_path(javafile, gfile) != 0){
...
int tp_gen_java_code(const char *save_dir, const char *file)
{
    ...
    cur_gfile = tpp_gfile_get_list();
    for (;cur_gfile != NULL; cur_gfile=cur_gfile->next){
        // Generate Protocol
        proto = tpp_protocol_tab_get(cur_gfile);
        for (cur_p = proto; cur_p != NULL; cur_p = cur_p->next){
            tp_gen_java_proto_decl(save_dir, cur_p, cur_gfile);
        }
    }
